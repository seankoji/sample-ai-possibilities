...
def fast_path_decision(
    ...
    # Fast path 3: Teammate has ball → Receiving 10% Shorter & Narrower (<5ms)
    ...
    if we_have_ball and possession_id != my_player_id:
        ...
        if position_label == "GK" or my_player_id == 0:
            ...
            return [{
                ...
                "parameters": {
                    "target_x": cb_x,
                    "target_y": max(-5.0, min(5.0, cb_shift_y)),
                    "sprint": False
                },
                "duration": 0
            }]
    ...
    # Fast path 4: Defensive Zonal Rest-Defense for CB/DEF (NEVER drops deeper than 18-yard line / into goal)
    if not we_have_ball and role_rules and getattr(role_rules, "own_half_only", False):
        cb_anchor_x = my_goal_x * 0.50
        cb_y = max(-6.0, min(6.0, ball_pos.get("y", 0) * 0.4))
        return [{
            "commandType": "MOVE_TO",
            "playerId": my_player_id,
            "teamId": team_id,
