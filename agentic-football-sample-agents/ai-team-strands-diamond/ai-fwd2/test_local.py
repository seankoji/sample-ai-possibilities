"""Local test for the Diamond ST agent — tests state summary, parsing, fallback, and LLM."""
...
def test_fallback():
    """ST doesn't have ball, teammate does — should MOVE_TO central forward pocket."""
...
def test_fallback_shoot():
    """ST has ball near opponent goal — should SHOOT TR."""
    ...
    state["players"][4]["position"] = {"x": 40, "y": 0}  # near opp goal (<20 from 55)
    ...
    print(f"  Correctly shoots (aim TR) near goal")
    print()
...
def test_parse():
    print("=== PARSE TESTS ===")
    tests = [
        ('[{"commandType":"SHOOT","playerId":4,"parameters":{"aim_location":"TR","power":0.85},"duration":0}]', 1),
        ('[{"commandType":"MOVE_TO","playerId":4,"parameters":{"target_x":33,"target_y":0,"sprint":false},"duration":0}]', 1),
        ("invalid json", 0),
        ('[]', 0),
    ]
    all_pass = True
