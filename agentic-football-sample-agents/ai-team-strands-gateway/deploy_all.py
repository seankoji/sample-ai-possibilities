...
The AgentCore Gateway and its four Lambda tactical tools are declared in
agentcore/agentcore.json ("tactical-tools") and are created by `agentcore
deploy` along with the agents. The CDK app builds the Lambdas from
gateway_tools/, creates the required IAM roles, and injects the gateway
endpoint into every runtime as AGENTCORE_GATEWAY_TACTICAL_TOOLS_URL — no
Lambda packaging, gateway setup, or GATEWAY_URL export is needed.
...
# CDK-flow variant of the gateway base module, staged into each agent dir
# under the name src/main.py imports (gateway_agent_base). The invoke handler
# reads no env vars, so both flows share the same file.
GATEWAY_BASE_SRC = SCRIPT_DIR / "gateway_agent_base_cdk.py"
GATEWAY_BASE_NAME = "gateway_agent_base.py"
INVOKE_HANDLER_SRC = SCRIPT_DIR / "gateway_invoke_handler.py"
INVOKE_HANDLER_NAME = "gateway_invoke_handler.py"
...
def cleanup():
    for agent_dir in _injected:
        ...
        if lib_copy.exists():
            rmtree(lib_copy)
        for name in (GATEWAY_BASE_NAME, INVOKE_HANDLER_NAME):
            copy = agent_dir / name
            if copy.exists():
                copy.unlink()
    if _injected:
...
print("==========================================")
print("  AI Team (Gateway) — Deploy Agents")
print("==========================================\n")
...
try:
    ...
    for agent in agents:
        ...
        shutil.copy2(GATEWAY_BASE_SRC, agent_dir / GATEWAY_BASE_NAME)
