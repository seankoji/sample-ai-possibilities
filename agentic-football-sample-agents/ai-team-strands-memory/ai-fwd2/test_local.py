"""Local test for the FWD2 (Memory) agent — tests state summary, parsing, and fallback."""
...
def test_parse():
    print("=== PARSE TESTS ===")
    tests = [
        ('[{"commandType":"SHOOT","playerId":4,"parameters":{"aim_location":"BL","power":0.85},"duration":0}]', 1),
        ("invalid json", 0),
    ]
...
def test_llm():
    ...
    mock_game_state_msg = (
        "Tick 42 | FWD2 in attacking third at (73,38). "
        "FWD1 making overlapping run at (76,30). Opponent DEF at (74,36). "
        "Decide: shoot bottom-left or lay off to FWD1?"
    )
