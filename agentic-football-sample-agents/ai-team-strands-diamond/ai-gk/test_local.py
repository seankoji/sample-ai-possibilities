"""Local test for the Diamond GK agent — tests state summary, parsing, fallback, and LLM."""
...
def test_fallback_with_ball():
    """Test fallback when GK has the ball — should GK_DISTRIBUTE to wide teammate (LM/RM)."""
    ...
    target = cmds[0]["parameters"].get("target_player_id")
    assert target in (2, 3), f"FAIL: GK should distribute to wide teammates (2 or 3), got {target}"
    print(f"  Correctly distributes ball to wide player {target} via {cmds[0]['parameters'].get('method')}")
    print()
...
def test_parse():
    print("=== PARSE TESTS ===")
    tests = [
        ('[{"commandType":"GK_DISTRIBUTE","playerId":0,"parameters":{"target_player_id":2,"method":"THROW"},"duration":0}]', 1),
